 ...
 * @default 24
 * 
 * @param Zero Damage Rate
 * @desc The default Zero Damage Rate when one is not specified.
 * Input a JavaScript or HTML Color
 * @default 0.75
 *
 * 
 * @param Weak Damage Rate
 * @desc The default Weak Damage Rate when one is not specified.
 * Input a JavaScript or HTML Color
 * @default 0.6
 * 
 * 
 * @param Normal Damage Rate
 * @desc The default Normal Damage Rate when one is not specified.
 * Input a JavaScript or HTML Color
 * @default 0.5
 * 
 * @param Outline Color
...
(function (_) {
	...
	_.zdr = String(params['Zero Damage Rate']);
	_.wdr = String(params['Weak Damage Rate']);
	_.ndr = String(params['Normal Damage Rate']);
	...
	_.organizeInfo = function (o) {
		if (o.type === _.TAS_ID) {
			...
			if (o.info.match(/Zero[ ]?Damage[ ]?Rate:\s*(.*)/im)) o.zdr = parseFloat(RegExp.$1);
			if (o.info.match(/Weak[ ]?Damage[ ]?Rate:\s*(.*)/im)) o.wdr = parseFloat(RegExp.$1);
			if (o.info.match(/Normal[ ]?Damage[ ]?Rate:\s*(.*)/im)) o.ndr = parseFloat(RegExp.$1);
	...
	TimedAttackSystem.prototype.initialize = function (x, y) {
		...
		this._window.margin = 0;
		this._window.padding = 0;
		this._window.contents.padding = 0;
		this._window.contents.margin = 0;
		this._window.standardPadding = function () { return 0; }
		this._window.contentsWidth = function () {
			return Graphics.boxWidth;
		};
		this._window.contents = new Bitmap(Graphics.boxWidth, Graphics.height)
		this._content = new Sprite(new Bitmap(Graphics.boxWidth, Graphics.height));
		this._content.margin = 0;
		this._content.padding = 0;
		this._content.openness = 0;
	...
	Object.defineProperty(TimedAttackSystem.prototype, 'contents', {
		...
		width: function () { return Graphics.boxWidth },
		configurable: true
	});
	...
	TimedAttackSystem.prototype.standardPadding = function () {
		return 0;
	};

	TimedAttackSystem.prototype.contentsWidth = function () {
		return Graphics.boxWidth;
	};
	...
	TimedAttackSystem.prototype.loadItem = function (item) {
		if (item.type === _.TAS_ID) {
			...
			this._wdr = item.wdr || parseFloat(params['Weak Damage Rate']);
			this._ndr = item.ndr || parseFloat(params['Normal Damage Rate']);
			this._zdr = item.zdr || parseFloat(params['Zero Damage Rate']);
			this._se = item.se;
	...
	TimedAttackSystem.prototype.playDefaultGame = function () {
		...
		this._window.contentsWidth = () => { return Graphics.boxWidth };
		if (this._notPressed) {
			var f = this._frame;
		...
		var areaY = 12;
		var areaH = 52;
		var dangerAreaWidth = Graphics.boxWidth;
		var normalAreaStart = (1 - this._wdr) * (Graphics.boxWidth / 2)
		var normalAreaWidth = Graphics.boxWidth - Graphics.boxWidth * (1 - this._wdr)
		var weakAreaStart = (1 - this._ndr) * (Graphics.boxWidth / 2)
		var weakAreaWidth = Graphics.boxWidth - Graphics.boxWidth * (1 - this._ndr)
		var zeroAreaStart = (Graphics.boxWidth / 2) - (Graphics.boxWidth / 2 * (1 - this._zdr));
		var zeroAreaWidth = (Graphics.boxWidth / 2 * (1 - this._zdr)) * 2;
		var spacingX = 9;

		this._window.contents.blt(bit, 0, 0, bit.width, bit.height, 0, 0, this._window.contentsWidth(), this._window.contentsHeight());
		this._window.contents.fillRect(spacingX, areaY, dangerAreaWidth - spacingX * 2, areaH, '#5b2d2d')
		this._window.contents.fillRect(normalAreaStart, areaY, normalAreaWidth, areaH, '#653764')
		this._window.contents.fillRect(weakAreaStart, areaY, weakAreaWidth, areaH, '#3a5c81')
		this._window.contents.fillRect(zeroAreaStart, areaY, zeroAreaWidth, areaH, '#529366')
		var x = this._xPosition;
		var entireLength = this.width;
		var target = eval(this._target);
		//this._window.drawText(Number(1 - (Math.abs(target - x) / (entireLength - target))).toFixed(1), 0, 40, Graphics.boxWidth, 'center')
		...
		if (eval(_.activateCondition) && this._notPressed) {
			...
			var entireLength = this.width;
			var target = eval(this._target);
			var power = Number(1 - (Math.abs(target - x) / (entireLength - target)));
			if (power >= this._zdr) {
				power = 0;
			} else if (power >= this._wdr && power < this._zdr) {
				power = 0.75
			}
			else if (power >= this._ndr && power < this._wdr) {
				power = 1
			}
			else {
				power = 2
